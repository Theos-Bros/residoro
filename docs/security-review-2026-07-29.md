...
## 8. CRITICAL, FIXED — Finding 7's root cause was systemic: present on 37 of 39 tables, and on `anon` everywhere (added and fully closed 2026-08-10)
...
**Fully closed (2026-08-10, later same day, `tb-platform-grant-lockdown-001`):** the deferred remainder above was scoped as a draft tracer bullet, then worked start to finish the same day. Four migrations, applied in sequence:
- `20260810210000_tier3_zero_policy_grant_lockdown.sql` — the six zero-RLS-policy tables (`contract_notifications`, `import_batches`, `imported_contacts`, `imported_properties`, `migration_temp_files`, `training_sessions`): `revoke all` from both roles, nothing re-granted. Pure hygiene — RLS already default-denied every command on all six regardless of grant.
- `20260810220000_profiles_workspaces_anon_lockdown.sql` — closed `anon` on `profiles`/`workspaces` specifically, the two tables Finding 7's own fixes never touched.
- `20260810230000_tier2_grant_lockdown.sql` — 10 tables with role/delegation/self-scoped RLS (`settings_edit_delegations`, all six `workspace_*_settings`, `listing_dockets`, `listing_share_events`, `notifications`): `revoke all` from both roles, precise re-grant per table.
- `20260810240000_tier1_grant_lockdown.sql` — the 15 highest-risk tables sharing `commission_earnings`' original shape (row-only tenant check, no column restriction): `contracts`, `closings`, `offers`, `listings`, `contacts`, `tasks`, `projects`, `properties`, `viewings`, `property_media`, `property_documents`, `buyer_requirements`, `buyer_requirement_matches`, `inquiries`, `project_unit_types`. Two tables (`buyer_requirement_matches`, `property_documents`) were caught missing from the tracer bullet's original inventory while starting the work — corrected before executing, not after.

Every column list across all four migrations was built by reading every real `getScopedClient(...).from(<table>)` call in `application/backend/src`, the same method Finding 7/8's first six tables used — not inferred from the schema or migration history. Live-verified with five new/updated scripts covering all 39 tables between them: `verify-financial-audit-grant-lockdown.ts` (re-run), `verify-tier2-grant-lockdown.ts`, `verify-tier1-transactions-grant-lockdown.ts` (the full viewing→offer→contract→closing deal flow through real routes, 12/12), `verify-tier1-escalation-checks.ts` (6/6), plus five pre-existing DoD scripts re-run clean (`verify-rls-scoped-client.ts`, `verify-buyer-leads-schema.ts`, `verify-buyer-leads-stage-tasks.ts`, `verify-buyer-leads-matching-project-units.ts`, `verify-property-media-external-links.ts`, `verify-brokerage-permissions-delegation.ts`) — two of which had pre-existing, unrelated staleness (a listing-activation script bug from a since-changed default status, and a signup-metadata assumption stale since Finding 1's own fix) fixed along the way rather than worked around.
