...
| 2.10.0 | 2026-08-11 | **Correction.** Supabase's security advisors flagged `generate_unique_handle(text)` (the function backing `handle`, above) as executable without signing in. It had never had an explicit grant/revoke statement, so it carried both Postgres's implicit default PUBLIC EXECUTE and Supabase's own default-privileges grant to `anon`/`authenticated` on new `public`-schema functions — the same un-revoked-default shape as v2.5.0/v2.7.0/v2.9.0's table-grant findings, this time on a function. It's only ever meant to be called internally by `handle_new_user()`; direct unauthenticated calls let a caller weakly enumerate whether a given email's derived handle already exists. Closed via `20260811100000_generate_unique_handle_grant_lockdown.sql` + a same-day follow-up (`20260811100001`) after the first revoke — targeting only the `PUBLIC` pseudo-role — turned out to leave `anon`/`authenticated`'s separately-recorded grants untouched. Live-verified: a direct anon RPC call now rejected (`42501`); the legitimate invite → `handle_new_user()` path still assigns a handle correctly (`verify-generate-unique-handle-grant-lockdown.ts`, 2/2). Correction to previously-inaccurate (silently-assumed) documentation, not a schema change, hence a minor bump per STD-002. |
